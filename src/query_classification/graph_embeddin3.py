...
def setup_performance_indexes():
    """
    Tạo database indexes để tăng tốc queries 10-20x
    CHỈ CHẠY 1 LẦN khi setup system
    """
    try:
        ...
        indexes = [
            ...
            "CREATE INDEX course_url_idx IF NOT EXISTS FOR (c:Course) ON (c.url)",
            "CREATE INDEX skill_name_idx IF NOT EXISTS FOR (s:Skill) ON (s.name)",
            "CREATE INDEX instructor_name_idx IF NOT EXISTS FOR (i:Instructor) ON (i.name)",
            ...
            "CREATE INDEX level_name_idx IF NOT EXISTS FOR (l:Level) ON (l.name)",
        ]
        
        results = []
        for index_query in indexes:
            try:
                # ✅ FIX: Sử dụng WRITE session thay vì READ session
                with neo4j_conn._driver.session(default_access_mode="WRITE") as session:
                ...
                index_name = index_query.split()[2] if len(index_query.split()) > 2 else "unknown"
                
                # Check if index already exists
                if "already exists" in str(e).lower() or "equivalent index already exists" in str(e).lower():
                    results.append(f"ℹ️ Exists: {index_name}")
                    logger.info(f"ℹ️ Index already exists: {index_name}")
                else:
                    results.append(f"❌ Failed: {index_name} - {str(e)}")
...
def check_existing_indexes():
    ...
    try:
        ...
        # ✅ FIX: Use WRITE session for SHOW INDEXES 
        with neo4j_conn._driver.session(default_access_mode="WRITE") as session:
            result = session.run("SHOW INDEXES")
            existing_indexes = [record.get('name', 'Unknown') for record in result]
            return existing_indexes
            
    except Exception as e:
        logger.error(f"Failed to check indexes: {e}")
        
        # Fallback: try with READ session (some Neo4j versions allow this)
        try:
            result = neo4j_conn.execute_query("CALL db.indexes()")
            existing_indexes = [record.get('description', 'Unknown') for record in result]
            return existing_indexes
        except Exception as e2:
            logger.error(f"Fallback check also failed: {e2}")
            return []
...
class Neo4jConnection:
    def __init__(self, uri, user, pwd, max_connection_pool_size=50):  # Tăng từ 25→50
        self._driver = GraphDatabase.driver(
            uri, 
            auth=(user, pwd),
            max_connection_pool_size=max_connection_pool_size,    # Tăng pool size
            connection_acquisition_timeout=15.0,                 # Giảm từ 15→10
            max_transaction_retry_time=15.0,                     # Giảm từ 15→10
            encrypted=False  # Tắt encryption cho local development
        )
    ...
    def _warm_up_connections(self):
        """Pre-establish connections để tránh cold start"""
        try:
...
class SBERTEmbeddingModel:
    def __init__(self, model_name="all-MiniLM-L6-v2"):
        ...
        # Thêm simple in-memory cache
        self.embedding_cache = {}
        self.cache_max_size = 1000
        
    @st.cache_data(ttl=3600, max_entries=100)  # Giảm từ 200→100
    ...
    def get_embedding(self, text: str) -> np.ndarray:
        """Optimized embedding với memory cache"""
        ...
        emb = self.get_embedding_cached(cache_key, text)
        
        # Store in memory cache với size limit
...
@st.cache_resource(max_entries=1)  # Giảm từ unlimited→1
...
def manage_chat_history():
    ...
    MAX_MESSAGES = 4  # Giảm từ 6→4 để tiết kiệm memory
    
    if len(st.session_state.messages) > MAX_MESSAGES:
        # Clear duplicate UI elements first
        duplicate_keys = [
            'current_results_displayed', 'duplicate_results', 'temp_analysis',
            'pending_results', 'pending_query', 'button_clicked'
        ]
        for key in duplicate_keys:
            if key in st.session_state:
                del st.session_state[key]
        ...
        try:
            if len(st.session_state.messages) % 10 == 0:  # Every 10 messages
                st.cache_data.clear()
        except:
            pass
...
with st.sidebar:
    ...
    with col1:
        if st.button("🗄️ Setup Indexes", help="Create database indexes for 10-20x faster queries"):
            with st.spinner("Creating indexes..."):
                results = setup_performance_indexes()
                success_count = len([r for r in results if "✅" in r])
                total_count = len(results)
                
                if success_count == total_count:
                    st.success(f"✅ All {total_count} indexes created!")
                elif success_count > 0:
                    st.warning(f"⚠️ {success_count}/{total_count} indexes created")
                else:
                    st.error("❌ Index creation failed")
    
    with col2:
        if st.button("📋 Check Indexes", help="View existing database indexes"):
            with st.spinner("Checking indexes..."):
                existing_indexes = check_existing_indexes()
                if existing_indexes:
                    st.success(f"Found {len(existing_indexes)} indexes")
    ...
    if st.button("📊 Memory Stats"):
        stats = {
            "Messages in session": len(st.session_state.get('messages', [])),
            "Cache entries": len(st.session_state.get('advisor', {}).embedding_model.embedding_cache) if hasattr(st.session_state.get('advisor', {}), 'embedding_model') else 0,
        }
        st.json(stats)
