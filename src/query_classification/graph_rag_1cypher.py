...
class LSTMEmbeddingModel:
    ...
    def train(self, texts: List[str]):
        """Huấn luyện mô hình trên cả tên và mô tả khóa học"""
        self.fit(texts)

    def get_semantic_similarity(self, text1: str, text2: str) -> float:
        """Tính độ tương đồng ngữ nghĩa giữa hai text"""
...
def handle_query_submission(query: str):
    ...
    try:
        with st.spinner("🔄 Processing..."):
            ...
            # Generate query plan first
            plan = st.session_state.advisor.query_processor.generate_query_plan(query)
            ...
            system_msg = {
                ...
                "metadata": {
                    ...
                    "query_plan": {  # Thêm phần này
                        "steps": plan.get('steps', []),
                        "final_query": plan.get('final_query', ''),
                        "debug_info": plan.get('debug_info', {})
                    }
                }
            }
        ...
        with st.chat_message(msg["role"], avatar=("👤" if msg["role"] == "user" else "🤖")):
            ...
            if "metadata" in msg:
                with st.expander("🔍 Query Execution Details"):
                    # Hiển thị thông tin query plan
                    if "query_plan" in msg["metadata"]:
                        qp = msg["metadata"]["query_plan"]
                        di = qp.get("debug_info", {})
                        
                        st.caption(f"🕒 Generated in {di.get('gen_time', 0):.2f}s | "
                                 f"Model: {di.get('model', 'N/A')} | "
                                 f"Tokens: {di.get('prompt_tokens', 0)}→{di.get('response_tokens', 0)}")
                        
                        st.subheader("Query Steps")
                        for step in qp.get("steps", []):
                            st.markdown(f"- {step}")
                        
                        st.subheader("Generated Cypher")
                        st.code(qp.get("final_query", ""), language="cypher")
                    
                    # Hiển thị metadata khác
